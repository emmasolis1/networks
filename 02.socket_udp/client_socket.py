...
server_name = '163.178.104.88' # Address of the server, it could be like ucr.ac.cr in which case the DNS will tranform into an IP or either the IP directly.
server_port = 13000 # The specific socket to which we want to connect because a server could be running multiple sockets for diferent programs.
client_socket = socket(AF_INET, SOCK_DGRAM) # Creates the socket. AF_INET == IPv4
message = input('Escriba un mensaje de saludo para enviar al server.')
